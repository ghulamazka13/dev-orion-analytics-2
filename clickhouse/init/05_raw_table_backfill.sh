...
if [ "$enable_raw_table_ingest" != "true" ]; then
  echo "Skipping raw-table parser backfill (ENABLE_RAW_TABLE_INGEST=$enable_raw_table_ingest)"
  exit 0
fi
...
if [ "${#existing_sources[@]}" -eq 0 ]; then
  echo "Skipping raw-table parser backfill (none of source tables exist)"
  exit 0
fi
...
sed -e "s|{{RAW_SOURCE_FROM}}|$raw_source_from|g" \
    -e "/^DROP TABLE IF EXISTS bronze\\..*;$/d" \
    -e "s|^CREATE MATERIALIZED VIEW bronze\\.suricata_events_mv$|INSERT INTO bronze.suricata_events_raw|" \
    -e "s|^CREATE MATERIALIZED VIEW bronze\\.wazuh_events_mv$|INSERT INTO bronze.wazuh_events_raw|" \
    -e "s|^CREATE MATERIALIZED VIEW bronze\\.zeek_events_mv$|INSERT INTO bronze.zeek_events_raw|" \
    -e "/^TO bronze\\..*$/d" \
    -e "/^AS$/d" \
    /docker-entrypoint-initdb.d/05_raw_table_ingest.sql.tmpl \
  | clickhouse-client "${client_args[@]}" --multiquery

echo "Applied raw-table parser backfill (idempotent) using source(s): ${existing_sources[*]}"
